...
struct sbus_priv {
	...
	long unsigned int gpio_wakeup_flags;
...
static void bes2600_gpio_wakeup_mcu(struct sbus_priv *self, int flag)
{
	...
	/* error check */
	if((self->gpio_wakeup_flags & BIT(flag)) != 0) {
	...
	gpio_wakeup = (self->gpio_wakeup_flags == 0);
	...
	self->gpio_wakeup_flags |= BIT(flag);
...
static void bes2600_gpio_allow_mcu_sleep(struct sbus_priv *self, int flag)
{
	...
	gpio_sleep = (self->gpio_wakeup_flags == 0);
