...
int wsm_epta_cmd(struct bes2600_common *hw_priv, struct wsm_epta_msg *arg)
{
	...
	if (1) { // The above if statement is always true, this causes a compiler error so just if (1) for now
...
void wsm_unlock_tx(struct bes2600_common *hw_priv)
{
	...
	if (hw_priv->bh_error)
		...
		if (tx_lock < 0) {
			/* We really should bug here but doing so makes bad unrecoverable things happen */
			//BUG_ON(1);
			atomic_add(1, &hw_priv->tx_lock);
			bes2600_err(BES2600_DBG_WSM, "[WSM] TX failed to unlock, already unlocked.\n");
			dump_stack();
		} else if (tx_lock == 0) {
			bes2600_bh_wakeup(hw_priv);
