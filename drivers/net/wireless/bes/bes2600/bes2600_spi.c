...
static int bes2600_platform_data_init(struct device *dev)
{
	...
	pdata->reset = of_get_named_gpiod_flags(np, "reset",
	...
	pdata->powerup = of_get_named_gpiod_flags(np, "powerup",
			0, (enum of_gpio_flags *)&config);
	if (gpio_is_valid(pdata->powerup)) {
	...
	pdata->irq_gpio = of_get_named_gpiod_flags(np, "irq_gpio", 0, (enum of_gpio_flags *)&config);
	...
	pdata->host_wakeup_wlan = of_get_named_gpiod_flags(np, "host_wakeup_wlan", 0, (enum of_gpio_flags *)&config);
